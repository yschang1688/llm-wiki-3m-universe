...
`raw/notebooklm/全民瘋AI-Python-機器學習-深度學習.md` 列 Transformer、自注意力、多頭與位置編碼等課程動線，支撐本頁「任務—長度—實作」三決策與常見錯誤中對位置與效率的提醒。`raw/notebooklm/IPAS-AI應用規劃師-中級初級通過筆記.md` 將 LLM 置於應用規劃末端，呼應父頁 [[Transformer架構]] 與 [[大語言模型]] 的產品脈絡。
...
- [[KVCache自回歸推論快取]]
- [[線性注意力與Mamba架構]]
- [[李宏毅2025生成式ML課程索引]] — 工程紀律的「解耦三軸」（算子／配比／任務形態）與課程索引的「五講群獨立索引」是「**軸解耦**」原則的工程-學習鏡像（batch-05 #D，與 batch-04 #C 訓練-推論軸解耦並列為寬解候選）
...
- 2026-05-05：採納 batch-05 #D（Transformer 架構核心機制 × 李宏毅 2025 索引）— `## 相關概念` 補連 [[李宏毅2025生成式ML課程索引]] 雙向，標記工程-學習軸解耦鏡像；與 batch-04 #C 累積 2 對軸解耦，作 #M1 寬解涵蓋的證據
